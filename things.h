...
inline bool pointInBox(const glm::vec4 p1,
					   const glm::vec2 p2) noexcept {
	return (p2.x >= p1.x&&p1.x + p1.z >= p2.x&&p1.y <= p2.y&&p1.y + p1.w >= p2.y);
}

inline bool boxInBox(const glm::vec2 p1, int width1, int height1,
					 const glm::vec2 p2, int width2, int height2) noexcept{
	return 
		(p2.x >= p1.x&&p1.x + width1 >= p2.x&&p1.y <= p2.y&&p1.y + height1 >= p2.y) ||
...
inline bool boxInBox(const glm::vec4 b1, const glm::vec4 b2)noexcept {
	return
...
inline bool circleInCircle(
	glm::vec2 p1,int r1,
	glm::vec2 p2,int r2)noexcept {
	return glm::length(p1 - p2) < r1 + r2;
}
inline bool inRange(const double a, double lower, double upper)noexcept {
	if (upper < lower)
		std::swap(upper, lower);
	return a >= lower && a <= upper;
}

inline constexpr double xclampy(const double lb, const double ub, const double i) noexcept {
	if (i>lb&&i<ub) {
		return i - ((lb + ub) / 2) >= 0 ? ub : lb;
	}return i;
}

inline glm::vec2 xclampy(const glm::vec2 lb, const glm::vec2 ub, const glm::vec2 i) noexcept {
	return { xclampy(lb.x,ub.x,i.x),xclampy(lb.y,ub.y,i.y) };
}

inline glm::vec3 xclampy(const glm::vec3 lb, const glm::vec3 ub, const glm::vec3 i)noexcept {
...
inline std::tuple<glm::vec2, bool> doLinesIntersect(const glm::vec2 p1s, const glm::vec2 p1e,
													const glm::vec2 p2s, const glm::vec2 p2e)noexcept {
...
inline std::tuple<glm::vec2, intersect> willIntersectBox(const glm::vec2 start, const glm::vec2 velocity, const glm::vec4 box)noexcept {//assumes start isnt inside box
...
template<typename T>
inline constexpr T clamp(T lb,T ub,T thing) noexcept {
	return thing < lb ? lb : ub < thing ? ub : thing;
}

template<typename T,typename comp>
inline constexpr T clamp(T lb, T ub, T thing,comp c)noexcept {
	return c(thing, lb) ? lb : c(ub, thing) ? ub : thing;
}

inline glm::vec2 unitVec(math::radians r)noexcept {
	return { math::cos(r),math::sin(r) };
}

inline glm::vec2 rotate(glm::vec2 dir, math::radians angle)noexcept {
